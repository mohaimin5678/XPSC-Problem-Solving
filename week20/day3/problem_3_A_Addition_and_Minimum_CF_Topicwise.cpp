...
void push(int n,int b,int e){
    ...
    //ei segment er all node e add hobe lazy er value
    t[n]+=lazy[n];    
...
ll query(int n,int b,int e,int i,int j){     //O(logN)
    ...
    if(e<i || b>j){   //"out of" segment
        return LLONG_MAX; //particular segment query range er baire
    }
    ...
    //just min(left,right) segment theke asha min add kore return
    //kore dicchi
}
void mohaimin(){
    int n,m;
